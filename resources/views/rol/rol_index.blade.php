            ...
            <!-- Si mostrarFormulario=false, se vuelve true -->
                <div v-if="!mostrarFormulario">
                    <div class="flex justify-between items-center mb-6">
                        <h2 class="text-2xl font-bold text-gray-800">Catalogo Roles</h2>
                        <button 
                            @click="nuevoRol"
                            ...
                            <table class="min-w-full leading-normal">
                                <thead>
                                    <tr class="bg-gray-700 text-white text-left text-xs uppercase font-semibold tracking-wider">
                                        ...
                                        <th class="px-5 py-3">Nombre del Rol</th>
                                ...
                                <tbody class="text-gray-700">
                                    ...
                                    <tr v-for="rol in roles" :key="rol.id" class="border-b border-gray-200 hover:bg-gray-50">
                                        <td class="px-5 py-5 text-sm">@{{ rol.id }}</td>
                                        <td class="px-5 py-5 text-sm font-bold">@{{ rol.nombre }}</td>
                                        <td class="px-5 py-5 text-sm text-gray-600">@{{ rol.descripcion }}</td>
                                    ...
                                    <tr v-if="roles.length === 0">
                                        <td colspan="4" class="px-5 py-5 text-center text-gray-500">No hay roles registrados.</td>
                                    </tr>
                                </tbody>
                            </table>
                            @if(session('success'))
                            ...
                            <div class="max-w-2xl mx-auto bg-white shadow-xl rounded-lg overflow-hidden">
                                 <div class="bg-gray-700 px-6 py-4 flex justify-between items-center">
                                    <h3 class="text-lg font-bold text-white">
                                        @{{ operacion==='Agregar'? 'Nuevo Rol':'Editar Rol ' + form.nombre}}
                                         
                                    </h3>
                                    <button @click="cancelar" class="text-gray-300 hover:text-white text-2xl">&times;</button>
                                </div>
                                  
                                <form method="POST" enctype="multipart/form-data" class="p-6" action="{{route('Rol.save')}}">
                                    @csrf
                                    <input type="hidden" name="id" :value="form.id">

                                    <div class="grid grid-cols-1 gap-6">
                                           <div>
                                                <label class="block text-gray-700 text-sm font-bold mb-2">Nombre rol</label>
